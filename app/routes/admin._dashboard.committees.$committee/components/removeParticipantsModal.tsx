...
const RemoveParticipantsModal = ({ close, committee }: { close: () => void, committee: CommitteeType }) => {
  const fetcher = useFetcher<any>()
  const delegates = committee.delegates.map(delegate => ({ id: delegate.id, name: delegate.user.name }))
  const [selectedDelegates, handleDelegateSelection, clearSelectedDelegates, handleSubmission] = useRemoveParticipants(fetcher, close, committee)
...
function useRemoveParticipants(fetcher: FetcherWithComponents<any>, close: () => void, committee: CommitteeType): [
