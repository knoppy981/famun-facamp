...
const FileFormModal = ({ close, user, selectedFiles, actionData, fetcher }:
  { close: () => void, user: UserType, selectedFiles: selectedFilesType, actionData: any, fetcher: FetcherWithComponents<any> }
) => {
  ...
  const [file, setFile, selectedFileName, setSelectedFileName, isImageUploaded, imagePreview, isDragging, onDragOver, onDragLeave, handleDrop, onFileChange] =
    useFileSubmission(fetcher.data, close, ref, user)
...
function useFileSubmission(actionData: any, close: () => void, ref: React.RefObject<HTMLInputElement>, user: UserType): [
  File | null,
  React.Dispatch<any>,
  string,
