...
const RemoveParticipantsModal = ({ close, delegation }: { close: () => void, delegation: DelegationType }) => {
  const fetcher = useFetcher<any>()
  const participants = delegation.participants?.map(p => ({ id: p.id, name: p.name }))
  const [selectedParticipants, handleParticipantSelection, clearSelectedParticipants, handleSubmission] = useChangeLeader(fetcher, delegation, close)
  const [fieldState, onSelectionChange, onInputChange, onOpenChange] = useComboBox(participants ?? [], handleParticipantSelection)
...
function useChangeLeader(fetcher: FetcherWithComponents<any>, delegation: DelegationType, close: () => void): [
...
function useComboBox(participants: { id: string; name: any; }[], handleParticipantSelection: (delegate: { id: string; name: string; }) => void): [
